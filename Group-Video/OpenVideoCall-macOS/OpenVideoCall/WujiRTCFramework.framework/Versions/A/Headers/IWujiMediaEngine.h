#ifndef WUJI_MEDIA_ENGINE_H
#define WUJI_MEDIA_ENGINE_H
#include <stdint.h>
...
class IAudioFrameObserver {
 public:
  ...
  struct AudioFrame {
    ...
    /** The timestamp of the external audio frame. You can use this parameter
 ...
 public:
  /** Retrieves the recorded audio frame.
   ...
   - false: Invalid buffer in AudioFrame, and the recorded audio frame is
   discarded.
   */
  ...
  /** Retrieves the audio playback frame for getting the audio.
   ...
   - false: Invalid buffer in AudioFrame, and the audio playback frame is
   discarded.
   */
  ...
  /** Retrieves the mixed recorded and playback audio frame.
   ...
   @param audioFrame Pointer to AudioFrame.
   @return
   - true: Valid buffer in AudioFrame and the mixed recorded and playback audio
   frame is sent out.
   - false: Invalid buffer in AudioFrame and the mixed recorded and playback
   audio frame is discarded.
   */
  ...
  */
  ...
  /** Determines whether to receive audio data from multiple channels.
   ...
   In the multi-channel scenario, if you want to get audio data from multiple
   channels, set the return value of this callback as true. After that, the SDK
   triggers the \ref IAudioFrameObserver::onPlaybackAudioFrameBeforeMixingEx
   "onPlaybackAudioFrameBeforeMixingEx" callback to send you the before-mixing
   audio data from various channels. You can also get the channel ID of each
   audio frame.

   @note
   - Once you set the return value of this callback as true, the SDK triggers
   only the \ref IAudioFrameObserver::onPlaybackAudioFrameBeforeMixingEx
   "onPlaybackAudioFrameBeforeMixingEx" callback to send the before-mixing audio
   frame. \ref IAudioFrameObserver::onPlaybackAudioFrameBeforeMixing
   "onPlaybackAudioFrameBeforeMixing" is not triggered. In the multi-channel
   scenario, Wuji recommends setting the return value as true.
   - If you set the return value of this callback as false, the SDK triggers
   only the `onPlaybackAudioFrameBeforeMixing` callback to send the audio data.
   @return
  ...
  After you successfully register the audio frame observer, if you set the
  return value of isMultipleChannelFrameWanted as true, the SDK triggers this
  callback each time it receives a before-mixing audio frame from any of the
  channel.
  ...
  - `false`: The data in AudioFrame in invalid, and do not send this audio
  frame.
  */
...
class IVideoFrameObserver {
 public:
  ...
  enum VIDEO_OBSERVER_POSITION {
    /**
     * 1: The post-capturer position, which corresponds to the video data in the
  ...
  /** Video frame information. The video data format is YUV420. The buffer
   * provides a pointer to a pointer. The interface cannot modify the pointer of
   * the buffer, but can modify the content of the buffer only.
   */
  struct VideoFrame {
    ...
    /** Set the rotation of this frame before rendering the video. Supports 0,
     * 90, 180, 270 degrees clockwise.
     */
    int rotation;  // rotation of this frame (0, 90, 180, 270)
    /** The timestamp of the external audio frame. It is mandatory. You can use
   this parameter for the following purposes:
     - Restore the order of the captured audio frame.
     - Synchronize audio and video frames in video-related scenarios, including
   scenarios where external video sources are used.
 ...
 public:
  /** Occurs each time the SDK receives a video frame captured by the local
   * camera.
   *
   * After you successfully register the video frame observer, the SDK triggers
   * this callback each time a video frame is received. In this callback, you
   * can get the video data captured by the local camera. You can then
   * pre-process the data according to your scenarios.
   *
   * After pre-processing, you can send the processed video data back to the SDK
   * by setting the `videoFrame` parameter in this callback.
   *
   * @note
   * This callback does not support sending processed RGBA video data back to
   * the SDK.
   *
   * @param videoFrame Pointer to VideoFrame.
   * @return Whether or not to ignore the current video frame if the
   * pre-processing fails:
   * - true: Do not ignore.
   * - false: Ignore the current video frame, and do not send it back to the
   * SDK.
   */
   ...
   *
   * After you successfully register the video frame observer, the SDK triggers
   * this callback each time when it receives a video frame. In this callback,
   * you can get the video data before encoding. You can then process the data
   * according to your particular scenarios.
   *
   * After processing, you can send the processed video data back to the SDK by
   * setting the `VideoFrame` parameter in this callback.
   *
   * @note
   * - As of v3.0.1, if you want to receive this callback, you also need to set
   * `POSITION_PRE_ENCODE(1 << 2)` as a frame position in the \ref
   * getObservedFramePosition "getObservedFramePosition" callback.
   * - The video data that this callback gets has been pre-processed, with its
   * content cropped, rotated, and the image enhanced.
   * - This callback does not support sending processed RGBA video data back to
   * the SDK.
   *
   ...
   * isMultipleChannelFrameWanted return false, the SDK triggers this callback
   * each time a video frame is received. In this callback, you can get the
   * video data sent by the remote user. You can then post-process the data
   * according to your scenarios.
   *
   * After post-processing, you can send the processed data back to the SDK by
   * setting the `videoFrame` parameter in this callback.
   *
   * @note
   * This callback does not support sending processed RGBA video data back to
  ...
  /** Occurs each time the SDK receives a video frame and prompts you to set the
   * video format.
   *
   * YUV420 is the default video format. If you want to receive other video
   * formats, register this callback in the IVideoFrameObserver class.
   *
   * After you successfully register the video frame observer, the SDK triggers
   * this callback each time it receives a video frame. You need to set your
   * preferred video data in the return value of this callback.
   *
  ...
  /** Occurs each time the SDK receives a video frame and prompts you whether or
   * not to rotate the captured video according to the rotation member in the
   * VideoFrame class.
   *
   * The SDK does not rotate the captured video by default. If you want to
   * rotate the captured video according to the rotation member in the
   * VideoFrame class, register this callback in the IVideoFrameObserver class.
   *
   * After you successfully register the video frame observer, the SDK triggers
   * this callback each time it receives a video frame. You need to set whether
   * or not to rotate the video frame in the return value of this callback.
   *
   * @note
  ...
  /** Occurs each time the SDK receives a video frame and prompts you whether or
   * not to mirror the captured video.
   *
   * The SDK does not mirror the captured video by default. Register this
   * callback in the IVideoFrameObserver class if you want to mirror the
   * captured video.
   *
   * After you successfully register the video frame observer, the SDK triggers
   * this callback each time a video frame is received. You need to set whether
   * or not to mirror the captured video in the return value of this callback.
   *
   * @note
   * This callback applies to RGBA video data only.
   *
  ...
  /** @since v3.0.0
   ...
   If you want the video frames acquired from \ref
   IVideoFrameObserver::onRenderVideoFrame "onRenderVideoFrame" to be more
   evenly spaced, you can register the `getSmoothRenderingEnabled` callback in
   the `IVideoFrameObserver` class and set its return value as `true`.

   @note
   - Register this callback before joining a channel.
   - This callback applies to scenarios where the acquired video frame is
   self-rendered after being processed, not to scenarios where the video frame
   is sent back to the SDK after being processed.
   ...
   * @since v3.0.1
   *
   * After you successfully register the video observer, the SDK triggers this
   * callback each time it receives a video frame. You can determine which
   * position to observe by setting the return value. The SDK provides 3
   * positions for observer. Each position corresponds to a callback function:
   * - `POSITION_POST_CAPTURER(1 << 0)`: The position after capturing the video
   * data, which corresponds to the \ref onCaptureVideoFrame
   * "onCaptureVideoFrame" callback.
   * - `POSITION_PRE_RENDERER(1 << 1)`: The position before receiving the remote
   * video data, which corresponds to the \ref onRenderVideoFrame
   * "onRenderVideoFrame" callback.
   * - `POSITION_PRE_ENCODER(1 << 2)`: The position before encoding the video
   * data, which corresponds to the \ref onPreEncodeVideoFrame
   * "onPreEncodeVideoFrame" callback.
   *
   * @note
   * - Use '|' (the OR operator) to observe multiple frame positions.
   * - This callback observes `POSITION_POST_CAPTURER(1 << 0)` and
   * `POSITION_PRE_RENDERER(1 << 1)` by default.
   * - To conserve the system consumption, you can reduce the number of frame
   * positions that you want to observe.
   *
  ...
  virtual uint32_t getObservedFramePosition() {
    return static_cast<uint32_t>(POSITION_POST_CAPTURER |
  ...
  /** Determines whether to receive video data from multiple channels.
   ...
   In the multi-channel scenario, if you want to get video data from multiple
   channels, set the return value of this callback as true. After that, the SDK
   triggers the onRenderVideoFrameEx callback to send you the video data from
   various channels. You can also get the channel ID of each video frame.

   @note
   - Once you set the return value of this callback as true, the SDK triggers
   only the `onRenderVideoFrameEx` callback to send the video frame.
   onRenderVideoFrame will not be triggered. In the multi-channel scenario, Wuji
   recommends setting the return value as true.
   - If you set the return value of this callback as false, the SDK triggers
   only the `onRenderVideoFrame` callback to send the video data.
   @return
  ...
  /** Gets the video frame from multiple channels.
   ...
   After you successfully register the video frame observer, if you set the
   return value of isMultipleChannelFrameWanted as true, the SDK triggers this
   callback each time it receives a video frame from any of the channel.

   You can process the video data retrieved from this callback according to your
   scenario, and send the processed data back to the SDK using the `videoFrame`
   ...
   */
  virtual bool onRenderVideoFrameEx(const char* channelId,
                                    unsigned int uid,
...
class IVideoFrame {
 public:
  ...
  /** Copies the frame.
   ...
   If the required size is larger than the allocated size, new buffers of the
  ...
  /** Converts the frame.
   ...
   - < 0: Failure.
   */
  virtual int convertFrame(VIDEO_TYPE dst_video_type,
                           int dst_sample_size,
...
struct ExternalVideoRenerContext {
   ...
   */
  void* view;
  /** Video display mode: \ref wuji::rtc::RENDER_MODE_TYPE "RENDER_MODE_TYPE" */
  int renderMode;
  /** The image layer location.
   ...
   @note If the value is set to below 0 or above 100, the #ERR_INVALID_ARGUMENT
   error occurs.
   */
  int zOrder;
...
class IMediaEngine {
 public:
  ...
  /** Registers an audio frame observer object.
   ...
   This method is used to register an audio frame observer object (register a
   callback). This method is required to register callbacks when the engine is
   required to provide an \ref IAudioFrameObserver::onRecordAudioFrame
   "onRecordAudioFrame" or \ref IAudioFrameObserver::onPlaybackAudioFrame
   "onPlaybackAudioFrame" callback.

   @param observer Audio frame observer object instance. If NULL is passed in,
  ...
  /** Registers a video frame observer object.
   ...
   You need to implement the IVideoFrameObserver class in this method, and
   register callbacks according to your scenarios.

   After you successfully register the video frame observer, the SDK triggers
   the registered callbacks each time a video frame is received.

   @note When handling the video data returned in the callbacks, pay attention
   to the changes in the `width` and `height` parameters, which may be adapted
   under the following circumstances:
   - When the network condition deteriorates, the video resolution decreases
   incrementally.
   - If the user adjusts the video profile, the resolution of the video returned
   in the callbacks also changes.
   @param observer Video frame observer object instance. If NULL is passed in,
   the registration is canceled.
   @return
   - 0: Success.
   - < 0: Failure.
  ...
  /** **DEPRECATED** */
  virtual int registerVideoRenderFactory(
      IExternalVideoRenderFactory* factory) = 0;
  /** **DEPRECATED** Use \ref
   wuji::media::IMediaEngine::pushAudioFrame(IAudioFrameObserver::AudioFrame*
   frame) "pushAudioFrame(IAudioFrameObserver::AudioFrame* frame)" instead.

   Pushes the external audio frame.

   @param type Type of audio capture device: #MEDIA_SOURCE_TYPE.
   @param frame Audio frame pointer: \ref IAudioFrameObserver::AudioFrame
   "AudioFrame".
   @param wrap Whether to use the placeholder. We recommend setting the default
   value.
   - true: Use.
  ...
  /** Pushes the external audio frame.
   ...
   @param frame Pointer to the audio frame: \ref IAudioFrameObserver::AudioFrame
   "AudioFrame".
   ...
   * \ref wuji::rtc::IRtcEngine::setExternalAudioSink
   ...
   *
   * @note
   * - Once you call the \ref wuji::media::IMediaEngine::pullAudioFrame
   ...
   * \ref wuji::media::IMediaEngine::pullAudioFrame "pullAudioFrame" method is
   * as follows:
  ...
  */
  ...
  /** Pushes the video frame using the \ref ExternalVideoFrame
   "ExternalVideoFrame" and passes the video frame to the Wuji SDK.

   @param frame Video frame to be pushed. See \ref ExternalVideoFrame
   "ExternalVideoFrame".

   @note In the Communication profile, this method does not support video frames
