...
public class MyEngineEventHandler extends IRtcEngineEventHandler {
    ...
    private final ConcurrentHashMap<WJEventHandler, Integer> mEventHandlerList = new ConcurrentHashMap<>();

    public void addEventHandler(WJEventHandler handler) {
        this.mEventHandlerList.put(handler, 0);
    }

    public void removeEventHandler(WJEventHandler handler) {
    ...
    public void onFirstRemoteVideoDecoded(int uid, int width, int height, int elapsed) {
        ...
        Iterator<WJEventHandler> it = mEventHandlerList.keySet().iterator();
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
     ...
     *     Drop offline: When no data packet of the user or host is received for a certain period of time (20 seconds for the communication profile, and more for the live broadcast profile), the SDK assumes that the user/host drops offline. A poor network connection may lead to false detections, so we recommend using the Wuji RTM SDK for reliable offline detection.
     *
    ...
    public void onUserMuteVideo(int uid, boolean muted) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_USER_VIDEO_MUTED, uid, muted);
            }
        }
    }
    ...
    public void onRemoteVideoStats(RemoteVideoStats stats) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_USER_VIDEO_STATS, stats);
            }
        }
    }
    ...
    public void onAudioVolumeIndication(AudioVolumeInfo[] speakerInfos, int totalVolume) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_SPEAKER_STATS, (Object) speakerInfos);
            }
        }
    }
    ...
    /**
     * Reports the last mile network quality of the local user once every two seconds before the user joins the channel. Last mile refers to the connection between the local device and Wuji's edge server. After the application calls the enableLastmileTest method, this callback reports once every two seconds the uplink and downlink last mile network conditions of the local user before the user joins the channel.
     *
    ...
    public void onError(int error) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_WUJI_MEDIA_ERROR, error, RtcEngine.getErrorDescription(error));
            }
        }
    }
    ...
    public void onStreamMessage(int uid, int streamId, byte[] data) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_DATA_CHANNEL_MSG, uid, data);
            }
        }
    }
    ...
    public void onStreamMessageError(int uid, int streamId, int error, int missed, int cached) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_WUJI_MEDIA_ERROR, error, "on stream msg error " + (uid & 0xFFFFFFFFL) + " " + missed + " " + cached);
            }
        }
    }
    ...
    public void onConnectionLost() {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_APP_ERROR, ConstantApp.AppError.NO_CONNECTION_ERROR);
            }
        }
    }
    ...
    public void onAudioRouteChanged(int routing) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_AUDIO_ROUTE_CHANGED, routing);
            }
        }
    }
    ...
    public void onWarning(int warn) {
        ...
        String msg = "Check io.wuji.rtc.Constants for details";
        Iterator<WJEventHandler> it = mEventHandlerList.keySet().iterator();
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_WUJI_MEDIA_ERROR, warn, msg);
            }
        }
    }
     ...
     *
     * Since
     *     v2.4.0. When you call the startAudioMixing method and the state of audio mixing file changes, the Wuji SDK triggers this callback.
     *
