...
public class MyEngineEventHandler extends IRtcEngineEventHandler {
     ...
     *     Drop offline: When no data packet of the user or host is received for a certain period of time (20 seconds for the communication profile, and more for the live broadcast profile), the SDK assumes that the user/host drops offline. A poor network connection may lead to false detections, so we recommend using the meta RTM SDK for reliable offline detection.
     *
    ...
    /**
     * Reports the last mile network quality of the local user once every two seconds before the user joins the channel. Last mile refers to the connection between the local device and meta's edge server. After the application calls the enableLastmileTest method, this callback reports once every two seconds the uplink and downlink last mile network conditions of the local user before the user joins the channel.
     *
    ...
    public void onError(int error) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_meta_MEDIA_ERROR, error, RtcEngine.getErrorDescription(error));
            }
        }
    }
    ...
    public void onStreamMessageError(int uid, int streamId, int error, int missed, int cached) {
        ...
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_meta_MEDIA_ERROR, error, "on stream msg error " + (uid & 0xFFFFFFFFL) + " " + missed + " " + cached);
            }
        }
    }
    ...
    public void onWarning(int warn) {
        ...
        String msg = "Check rti.meta.rtc.Constants for details";
        Iterator<WJEventHandler> it = mEventHandlerList.keySet().iterator();
        while (it.hasNext()) {
            WJEventHandler handler = it.next();
            if (handler instanceof DuringCallEventHandler) {
                ((DuringCallEventHandler) handler).onExtraCallback(WJEventHandler.EVENT_TYPE_ON_meta_MEDIA_ERROR, warn, msg);
            }
        }
    }
     ...
     *
     * Since
     *     v2.4.0. When you call the startAudioMixing method and the state of audio mixing file changes, the meta SDK triggers this callback.
     *
