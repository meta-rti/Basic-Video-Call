...
enum LOCAL_INVITATION_STATE {
  ...
  /**
   0: <b>RETURNED TO THE CALLER.</b> The initial state of a call invitation
   (idle).
   */
  ...
  /**
   2: <b>RETURNED TO THE CALLER.</b> The call invitation is received by the
   callee.
   */
  LOCAL_INVITATION_STATE_RECEIVED_BY_REMOTE = 2,

  /**
   3: <b>RETURNED TO THE CALLER.</b> The call invitation is accepted by the
   callee.
   */
  LOCAL_INVITATION_STATE_ACCEPTED_BY_REMOTE = 3,

  /**
   4: <b>RETURNED TO THE CALLER.</b> The call invitation is declined by the
   callee.
   */
...
enum REMOTE_INVITATION_STATE {
  ...
  /**
   0: <b>RETURNED TO THE CALLEE.</b> The initial state of a call invitation
   (idle).
   */
  REMOTE_INVITATION_STATE_IDLE = 0,

  /**
   1: <b>RETURNED TO THE CALLEE.</b> A call invitation from a remote caller is
   received.
   */
  REMOTE_INVITATION_STATE_INVITATION_RECEIVED = 1,

  /**
   2: <b>RETURNED TO THE CALLEE.</b> The message is sent to the caller that the
  ...
  /**
   5: <b>RETURNED TO THE CALLEE.</b> The call invitation is canceled by the
   remote caller.
   */
...
/**
 @brief <b>RETURNED TO THE CALLER.</b> Error codes of an outgoing call
 invitation.
 */
enum LOCAL_INVITATION_ERR_CODE {
  ...
  /**
   ...
   - Keeps resending the call invitation to the callee, if the callee is
   offline.
   - Returns this error code, if the callee is still offline 30 seconds since
   the call invitation is sent.
   */
  LOCAL_INVITATION_ERR_PEER_OFFLINE = 1,

  /**
   2: <b>RETURNED TO THE CALLER.</b> The callee is online but has not ACKed to
   the call invitation 30 seconds since it is sent.
   */
  LOCAL_INVITATION_ERR_PEER_NO_RESPONSE = 2,

  /**
   3: <b>RETURNED TO THE CALLER. SAVED FOR FUTURE USE.</b> The call invitation
   expires 60 seconds since it is sent, if the callee ACKs to the call
   invitation but neither the caller or callee takes any further action (cancel,
   accpet, or decline it).
   */
...
/**
 @brief <b>RETURNED TO THE CALLEE.</b> Error codes of an incoming call
 invitation.
 */
enum REMOTE_INVITATION_ERR_CODE {
  ...
  /**
   1: <b>RETURNED TO THE CALLEE.</b> The call invitation received by the callee
   fails: the callee is not online.
   */
  REMOTE_INVITATION_ERR_PEER_OFFLINE = 1,

  /**
   2: <b>RETURNED TO THE CALLEE.</b> The call invitation received by callee
   fails: The callee does not ACK within a set time after the callee accepts the
   call invitation. This is usually a result of a network interruption.
   */
  REMOTE_INVITATION_ERR_ACCEPT_FAILURE = 2,

  /**
   3: <b>RETURNED TO THE CALLEE.</b> The call invitation expires 60 seconds
   since it is sent, if the callee ACKs to the call invitation but neither the
   caller or callee takes any further action (cancel, accpet, or decline it).
   */
...
enum INVITATION_API_CALL_ERR_CODE {
  ...
  /**
   1: The method call fails. The argument is invalid, for example, the value of
   the @p content parameter exceeds 8K bytes.
   */
  ...
  INVITATION_API_CALL_ERR_ALREADY_END =
      3,  // accepted, failure, canceled, refused
...
/**
 The class allowing the caller to retrieve the attributes of an outgoing call
 invitation.
 */
class ILocalCallInvitation {
 ...
 public:
  ...
  /**
   ...
   @param content The content of the call invitation. The @p content must not
  ...
  /**
   ...
   wuji::rtm::ILocalCallInvitation::setContent "setContent" method.
   */
  ...
  /**
   ...
   @note To intercommunicate with the legacy Wuji Signaling SDK, you MUST set
   the channel ID. However, even if the callee successfully accepts the call
   invitation, the Wuji RTM SDK does not join the channel of the specified
   channel ID.
  ...
  /**
   ...
   wuji::rtm::IRemoteCallInvitation::setResponse "setResponse" method.
   */
...
/**
 The class allowing the callee to retrieve the attributes of an incoming call
 invitation.
 */
class IRemoteCallInvitation {
 ...
 public:
  ...
  /**
   ...
   @param response The callee's response to the call invitation. The @p response
   must not exceed 8 KB in length if encoded in UTF-8.
   */
  ...
  /**
   ...
   @note The callee sets a response to the incoming call invitation using the
   \ref wuji::rtm::IRemoteCallInvitation::setResponse "setResponse" method.
   */
  virtual const char* getResponse() const = 0;
  ...
  /**
   ...
   @return The state of the incoming call invitation See:
...
class IRtmCallManager {
 ...
 public:
  /**
   ...
   @param invitation An \ref wuji::rtm::ILocalCallInvitation
   "ILocalCallInvitation" object.
   @return
   - 0: Success.
  ...
  /**
   ...
   @param invitation An \ref wuji::rtm::IRemoteCallInvitation
   "IRemoteCallInvitation" object.
   @return
   - 0: Success.
   - <0: Failure. See #INVITATION_API_CALL_ERR_CODE for the error code.
   */
  ...
  /**
   Creates an ILocalCallInvitation object.
       ...
       @return An \ref wuji::rtm::ILocalCallInvitation "ILocalCallInvitation"
   object.
   */
  virtual ILocalCallInvitation* createLocalCallInvitation(
      const char* calleeId) = 0;
