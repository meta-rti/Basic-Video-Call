...
enum INTERFACE_ID_TYPE {
  WUJI_IID_AUDIO_DEVICE_MANAGER = 1,
  WUJI_IID_VIDEO_DEVICE_MANAGER = 2,
  WUJI_IID_RTC_ENGINE_PARAMETER = 3,
  WUJI_IID_MEDIA_ENGINE = 4,
  WUJI_IID_SIGNALING_ENGINE = 8,
};
...
enum WARN_CODE_TYPE {
   ...
   */
  WARN_INVALID_VIEW = 8,
  /** 16: Failed to initialize the video function, possibly caused by a lack of
   * resources. The users cannot see the video while the voice communication is
   * not affected.
   */
  WARN_INIT_VIDEO = 16,
  /** 20: The request is pending, usually due to some module not being ready,
   * and the SDK postponed processing the request.
   */
  WARN_PENDING = 20,
   ...
   * the SDK looks up the specified channel. This warning usually occurs when
   * the network condition is too poor for the SDK to connect to the server.
   */
  WARN_LOOKUP_CHANNEL_TIMEOUT = 104,
  /** **DEPRECATED** 105: The server rejects the request to look up the channel.
   The server cannot process this request or the request is illegal.

   Deprecated as of v2.4.1. Use CONNECTION_CHANGED_REJECTED_BY_SERVER(10) in the
   \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged
   "onConnectionStateChanged" callback instead.
  */
   ...
   * network condition is too poor for the SDK to connect to the server.
   */
  WARN_OPEN_CHANNEL_TIMEOUT = 106,
  /** 107: The server rejects the request to open the channel. The server cannot
  ...
  /** 118: A timeout occurs when setting the client role in the live broadcast
   * profile.
   */
  ...
  /** 1025: The audio playback or recording is interrupted by system events
  ...
  /** 1042: Audio device module: The audio recording device is different from
   * the audio playback device, which may cause echoes problem. Wuji recommends
   * using the same audio device to record and playback audio.
   */
  WARN_ADM_INCONSISTENT_AUDIO_DEVICE = 1042,
  /** 1051: (Communication profile only) audio Processing Module: howling is
   * detected.
   */
  ...
  /** 1610: Super-resolution warning: the original video dimensions of the
   * remote user exceed 640 * 480.
   */
...
enum ERROR_CODE_TYPE {
  ...
  /** 9: No permission exists. Check if the user has granted access to the audio
   * or video device.
   */
  ERR_NO_PERMISSION = 9,
  /** 10: An API method timeout occurs. Some API methods require the SDK to
   * return the execution result, and this error occurs if the request takes too
   * long (more than 10 seconds) for the SDK to process.
   */
  ERR_TIMEDOUT = 10,
  /** 11: The request is canceled. This is for internal SDK use only, and it
   * does not return to the application through any method or callback.
   */
  ERR_CANCELED = 11,
  /** 12: The method is called too often. This is for internal SDK use only, and
   * it does not return to the application through any method or callback.
   */
  ERR_TOO_OFTEN = 12,
   ...
   * the channel, for example, \ref wuji::rtc::IRtcEngine::joinChannel
   * "joinChannel".
   */
  ...
  /** 18: The request to leave the channel is rejected.
   ...
   - When the user has left the channel and still calls \ref
   wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" to leave the channel. In
   this case, stop calling \ref wuji::rtc::IRtcEngine::leaveChannel
   "leaveChannel".
   - When the user has not joined the channel and still calls \ref
   wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" to leave the channel. In
   this case, no extra operation is needed.
   */
  ...
  /** **DEPRECATED** 109: Deprecated as of v2.4.1. Use
   CONNECTION_CHANGED_TOKEN_EXPIRED(9) in the \ref
   ...
   seconds elapsed since 1/1/1970. The user can use the Token to access the Wuji
   service within 24 hours after the Token is generated. If the user does not
   access the Wuji service after 24 hours, this Token is no longer valid.
   - Call Expiration Timestamp expired: The timestamp is the exact time when a
   user can no longer use the Wuji service (for example, when a user is forced
   to leave an ongoing call). When a value is set for the Call Expiration
   Timestamp, it does not mean that the token will expire, but that the user
   will be banned from the channel.
   */
  ERR_TOKEN_EXPIRED = 109,
  /** **DEPRECATED** 110: Deprecated as of v2.4.1. Use
   CONNECTION_CHANGED_INVALID_TOKEN(8) in the \ref
   wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged
   "onConnectionStateChanged" callback instead.

   The token is invalid due to one of the following reasons:

   - The App Certificate for the project is enabled in Console, but the user is
   still using the App ID. Once the App Certificate is enabled, the user must
   use a token.
   - The uid is mandatory, and users must set the same uid as the one set in the
   \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method.
   */
  ERR_INVALID_TOKEN = 110,
  /** 111: The internet connection is interrupted. This applies to the Wuji Web
   * SDK only.
   */
  ERR_CONNECTION_INTERRUPTED = 111,  // only used in web sdk
  /** 112: The internet connection is lost. This applies to the Wuji Web SDK
   * only.
   */
  ERR_CONNECTION_LOST = 112,  // only used in web sdk
  /** 113: The user is not in the channel when calling the \ref
   * wuji::rtc::IRtcEngine::sendStreamMessage "sendStreamMessage" or \ref
   * wuji::rtc::IRtcEngine::getUserInfoByUserAccount "getUserInfoByUserAccount"
   * method.
   */
  ERR_NOT_IN_CHANNEL = 113,
  /** 114: The size of the sent data is over 1024 bytes when the user calls the
   * \ref wuji::rtc::IRtcEngine::sendStreamMessage "sendStreamMessage" method.
   */
  ERR_SIZE_TOO_LARGE = 114,
  /** 115: The bitrate of the sent data exceeds the limit of 6 Kbps when the
   * user calls the \ref wuji::rtc::IRtcEngine::sendStreamMessage
   * "sendStreamMessage" method.
   */
  ERR_BITRATE_LIMIT = 115,
  /** 116: Too many data streams (over 5 streams) are created when the user
   * calls the \ref wuji::rtc::IRtcEngine::createDataStream "createDataStream"
   * method.
   */
  ...
  /** 130: Encryption is enabled when the user calls the \ref
   * wuji::rtc::IRtcEngine::addPublishStreamUrl "addPublishStreamUrl" method
  ...
  /** 151: CDN related errors. Remove the original URL address and add a new one
   * by calling the \ref wuji::rtc::IRtcEngine::removePublishStreamUrl
   * "removePublishStreamUrl" and \ref
   * wuji::rtc::IRtcEngine::addPublishStreamUrl "addPublishStreamUrl" methods.
   */
  ERR_PUBLISH_STREAM_CDN_ERROR = 151,
  /** 152: The host publishes more than 10 URLs. Delete the unnecessary URLs
  ...
  /** 154: An error occurs in Wuji's streaming server. Call the
  ...
  /** 156: The format of the RTMP stream URL is not supported. Check whether the
  ...
  Deprecated as of v2.4.1. Use LOCAL_VIDEO_STREAM_ERROR_CAPTURE_FAILURE(4) in
  ...
  /** 1009: Audio Device Module: An error occurs in starting the playback
   * device.
   */
  ERR_ADM_START_PLAYOUT = 1009,
  /** 1010: Audio Device Module: An error occurs in stopping the playback
   * device.
   */
  ...
  /** 1012: Audio Device Module: An error occurs in starting the recording
   * device.
   */
  ERR_ADM_START_RECORDING = 1012,
  /** 1013: Audio Device Module: An error occurs in stopping the recording
   * device.
   */
...
Deprecated as of v2.4.1. Use LOCAL_VIDEO_STREAM_ERROR_DEVICE_BUSY(3) in the \ref
wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged
"onConnectionStateChanged" callback instead.
   */
