...
enum RTMP_STREAM_PUBLISH_STATE
{
  ...
  /** The SDK is connecting to Wuji's streaming server and the RTMP server. This state is triggered after you call the \ref IRtcEngine::addPublishStreamUrl "addPublishStreamUrl" method.
   */
...
enum RTMP_STREAM_PUBLISH_ERROR
{
  ...
  /** An error occurs in Wuji's streaming server. Call the addPublishStreamUrl method to publish the streaming again. */
...
{
    /** 0: Read-only mode: Users only read the \ref wuji::media::IAudioFrameObserver::AudioFrame "AudioFrame" data without modifying anything. For example, when users acquire the data with the Wuji SDK, then push the RTMP streams. */
    RAW_AUDIO_FRAME_OP_MODE_READ_ONLY = 0,
    /** 1: Write-only mode: Users replace the \ref wuji::media::IAudioFrameObserver::AudioFrame "AudioFrame" data with their own data and pass the data to the SDK for encoding. For example, when users acquire the data. */
    RAW_AUDIO_FRAME_OP_MODE_WRITE_ONLY = 1,
    /** 2: Read and write mode: Users read the data from \ref wuji::media::IAudioFrameObserver::AudioFrame "AudioFrame", modify it, and then play it. For example, when users have their own sound-effect processing module and perform some voice pre-processing, such as a voice change. */
...
enum AUDIO_REVERB_PRESET {
     ...
     * To achieve better virtual stereo reverberation, Wuji recommends setting `profile` in `setAudioProfile`
...
enum CONNECTION_STATE_TYPE
{
  /** 1: The SDK is disconnected from Wuji's edge server.
   ...
   - This is the initial state before calling the \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method.
   - The SDK also enters this state when the application calls the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method.
   */
  CONNECTION_STATE_DISCONNECTED = 1,
  /** 2: The SDK is connecting to Wuji's edge server.

   - When the application calls the \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method, the SDK starts to establish a connection to the specified channel, triggers the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged "onConnectionStateChanged" callback, and switches to the #CONNECTION_STATE_CONNECTING state.
   - When the SDK successfully joins the channel, it triggers the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged "onConnectionStateChanged" callback and switches to the #CONNECTION_STATE_CONNECTED state.
   - After the SDK joins the channel and when it finishes initializing the media engine, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onJoinChannelSuccess "onJoinChannelSuccess" callback.
   */
  CONNECTION_STATE_CONNECTING = 2,
  /** 3: The SDK is connected to Wuji's edge server and has joined a channel. You can now publish or subscribe to a media stream in the channel.
   ...
   - The \ref wuji::rtc::IRtcEngineEventHandler::onConnectionInterrupted "onConnectionInterrupted" callback (deprecated).
   - The \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged "onConnectionStateChanged" callback and switches to the #CONNECTION_STATE_RECONNECTING state.
   */
  CONNECTION_STATE_CONNECTED = 3,
  /** 4: The SDK keeps rejoining the channel after being disconnected from a joined channel because of network issues.

   - If the SDK cannot rejoin the channel within 10 seconds after being disconnected from Wuji's edge server, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionLost "onConnectionLost" callback, stays in the #CONNECTION_STATE_RECONNECTING state, and keeps rejoining the channel.
   - If the SDK fails to rejoin the channel 20 minutes after being disconnected from Wuji's edge server, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged "onConnectionStateChanged" callback, switches to the #CONNECTION_STATE_FAILED state, and stops rejoining the channel.
   */
  CONNECTION_STATE_RECONNECTING = 4,
  /** 5: The SDK fails to connect to Wuji's edge server or join the channel.

   You must call the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method to leave this state, and call the \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method again to rejoin the channel.

   If the SDK is banned from joining the channel by Wuji's edge server (through the RESTful API), the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionBanned "onConnectionBanned" (deprecated) and \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged "onConnectionStateChanged" callbacks.
   */
...
enum CONNECTION_CHANGED_REASON_TYPE
{
  ...
  /** 8: The connection failed since token is not valid, possibly because:
   ...
   - The uid that you specify in the \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method is different from the uid that you pass for generating the token.
   */
  ...
  /** 14: Timeout for the keep-alive of the connection between the SDK and Wuji's edge server. The connection state changes to CONNECTION_STATE_RECONNECTING(4). */
...
struct AudioVolumeInfo
{
    ...
    */
     ...
     * - Ensure that you set `report_vad`(true) in the \ref wuji::rtc::IRtcEngine::enableAudioVolumeIndication(int, int, bool) "enableAudioVolumeIndication" method to enable the voice activity detection of the local user.
     */
    unsigned int vad;
...
/** The video and audio properties of the user displaying the video in the CDN live. Wuji supports a maximum of 17 transcoding users in a CDN streaming channel.
*/
...
typedef struct LiveTranscoding {
    ...
    */
	...
	@note If you set this parameter to other values, Wuji adjusts it to the default value of 100.
    */
    ...
    */
    ...
    /** Reserved property. Extra user-defined information to send SEI for the H.264/H.265 video stream to the CDN live client. Maximum length: 4096 Bytes.
     ...
     For more information on SEI frame, see [SEI-related questions](https://docs.wuji.io/en/faq/sei).
     */
    ...
    */
     ...
     */
    int audioBitrate;
    /** The numbder of audio channels for the CDN live stream. Wuji recommends choosing 1 (mono), or 2 (stereo) audio channels. Special players are required if you choose option 3, 4, or 5:
...
struct ScreenCaptureParameters
{
    /** The maximum encoding dimensions of the shared region in terms of width * height.
	 ...
	 The default value is 1920 * 1080 pixels, that is, 2073600 pixels. Wuji uses the value of this parameter to calculate the charges.

	 If the aspect ratio is different between the encoding dimensions and screen dimensions, Wuji applies the following algorithms for encoding. Suppose the encoding dimensions are 1920 x 1080:
 ...
 */
	struct Packet
	{
         ...
         * @note Wuji recommends that the value of buffer is more than 2048 bytes, otherwise, you may meet undefined behaviors such as a crash.
         */
...
public:
    ...
    The SDK triggers this callback when the local user switches the user role by calling the \ref wuji::rtc::IRtcEngine::setClientRole "setClientRole" method after joining the channel.
    ...
    /** Occurs when a remote user (Communication)/ host (Live Broadcast) joins the channel.
     ...
     - A remote user/host joins the channel by calling the \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method.
     - A remote user switches the user role to the host by calling the \ref wuji::rtc::IRtcEngine::setClientRole "setClientRole" method after joining the channel.
     - A remote user/host rejoins the channel after a network interruption.
     - The host injects an online media stream into the channel by calling the \ref wuji::rtc::IRtcEngine::addInjectStreamUrl "addInjectStreamUrl" method.
    ...
    - Drop offline: When no data packet of the user or host is received for a certain period of time, the SDK assumes that the user/host drops offline. Unreliable network connections may lead to false detections, so we recommend using the Wuji RTM SDK for more reliable offline detection.
    ...
    /** Reports the last mile network quality of the local user once every two seconds before the user joins the channel.
     ...
     Last mile refers to the connection between the local device and Wuji's edge server. After the application calls the \ref IRtcEngine::enableLastmileTest "enableLastmileTest" method, this callback reports once every two seconds the uplink and downlink last mile network conditions of the local user before the user joins the channel.
    ...
    The SDK triggers this callback within 30 seconds after the app calls the \ref wuji::rtc::IRtcEngine::startLastmileProbeTest "startLastmileProbeTest" method.
    ...
    */
    ...
    /** **DEPRECATED** Occurs when the connection between the SDK and the server is interrupted.
     ...
     Deprecated as of v2.3.2. Replaced by the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged "onConnectionStateChanged(CONNECTION_STATE_RECONNECTING, CONNECTION_CHANGED_INTERRUPTED)" callback.
     ...
     This callback is different from \ref wuji::rtc::IRtcEngineEventHandler::onConnectionLost "onConnectionLost":
     - The SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionInterrupted "onConnectionInterrupted" callback when it loses connection with the server for more than four seconds after it successfully joins the channel.
     - The SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionLost "onConnectionLost" callback when it loses connection with the server for more than 10 seconds, whether or not it joins the channel.
    ...
    */
    ...
    This callback is different from \ref wuji::rtc::IRtcEngineEventHandler::onConnectionInterrupted "onConnectionInterrupted":
    ...
    /** **DEPRECATED** Deprecated as of v2.3.2. Replaced by the \ref wuji::rtc::IRtcEngineEventHandler::onConnectionStateChanged "onConnectionStateChanged(CONNECTION_STATE_FAILED, CONNECTION_CHANGED_BANNED_BY_SERVER)" callback.
    ...
    /** Occurs when the token expires.
     ...
     After a token is specified by calling the \ref IRtcEngine::joinChannel "joinChannel" method, if the SDK losses connection with the Wuji server due to network issues, the token may expire after a certain period of time and a new token may be required to reconnect to the server.
    ...
    Deprecated as of v2.3.2. Use the \ref wuji::rtc::IRtcEngineEventHandler::onRemoteAudioStats "onRemoteAudioStats" callback instead.
    ...
    /** Reports the last mile network quality of each user in the channel once every two seconds.
     ...
     Last mile refers to the connection between the local device and Wuji's edge server. This callback reports once every two seconds the last mile network conditions of each user in the channel. If a channel includes multiple users, the SDK triggers this callback as many times.
   ...
   virtual void onLocalVideoStats(const LocalVideoStats& stats) {
    ...
    /** Reports the statistics of the audio stream from each remote user/host.
     ...
     This callback replaces the \ref wuji::rtc::IRtcEngineEventHandler::onAudioQuality "onAudioQuality" callback.
    ...
    /** Reports which users are speaking, the speakers' volume and whether the local user is speaking.
     ...
     - Calling the \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream" method affects the SDK's behavior:
        - If the local user calls the \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream" method, the SDK stops triggering the local user's callback.
    ...
    */
    ...
    /** **DEPRECATED** Occurs when the video stops playing.
     ...
     Deprecated as of v2.4.1. Use LOCAL_VIDEO_STREAM_STATE_STOPPED(0) in the \ref wuji::rtc::IRtcEngineEventHandler::onLocalVideoStateChanged "onLocalVideoStateChanged" callback instead.
     */
    ...
    */
    ...
    /** @deprecated This method is deprecated from v3.0.0, use the \ref wuji::rtc::IRtcEngineEventHandler::onRemoteAudioStateChanged "onRemoteAudioStateChanged" callback instead.
     
     Occurs when a remote user's audio stream playback pauses/resumes.

     The SDK triggers this callback when the remote user stops or resumes sending the audio stream by calling the \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream" method.
    ...
    /** **DEPRECATED** Occurs when the camera turns on and is ready to capture the video.
     ...
     Deprecated as of v2.4.1. Use #LOCAL_VIDEO_STREAM_STATE_CAPTURING (1) in the \ref wuji::rtc::IRtcEngineEventHandler::onLocalVideoStateChanged "onLocalVideoStateChanged" callback instead.
     */
     ...
     * - On Android, the `distance` value reported in this callback may be slightly different from the actual distance. Therefore, Wuji does not recommend using it for
    ...
    /**
     ...
     - The remote user stops sending the audio stream and re-sends it after 15 seconds. Reasons for such an interruption include:
         ...
         - The remote user drops offline.
         - The remote user calls the \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream" method to stop sending the local audio stream.
         - The remote user calls the \ref wuji::rtc::IRtcEngine::disableAudio "disableAudio" method to disable audio.
...
//    virtual void onStreamError(int streamId, int code, int parameter, const char* message, size_t length) {}
    ...
    The SDK triggers this callback when the local user receives the stream message that the remote user sends by calling the \ref wuji::rtc::IRtcEngine::sendStreamMessage "sendStreamMessage" method.
    ...
    */
	...
	/** Occurs when the local user does not receive the data stream from the remote user within five seconds.
     ...
     The SDK triggers this callback when the local user fails to receive the stream message that the remote user sends by calling the \ref wuji::rtc::IRtcEngine::sendStreamMessage "sendStreamMessage" method.
     @param uid User ID of the remote user sending the message.
     @param streamId Stream ID.
  ...
  /**
   ...
   The SDK triggers this callback to report the result of the local user calling the \ref wuji::rtc::IRtcEngine::addPublishStreamUrl "addPublishStreamUrl" or \ref wuji::rtc::IRtcEngine::removePublishStreamUrl "removePublishStreamUrl" method.
  ...
  virtual void onRtmpStreamingStateChanged(const char *url, RTMP_STREAM_PUBLISH_STATE state, RTMP_STREAM_PUBLISH_ERROR errCode) {
    ...
    /** @deprecated This method is deprecated, use the \ref wuji::rtc::IRtcEngineEventHandler::onRtmpStreamingStateChanged "onRtmpStreamingStateChanged" callback instead.
     ...
     - #ERR_INVALID_ARGUMENT (2): Invalid argument used. If, for example, you did not call \ref wuji::rtc::IRtcEngine::setLiveTranscoding "setLiveTranscoding" to configure LiveTranscoding before calling \ref wuji::rtc::IRtcEngine::addPublishStreamUrl "addPublishStreamUrl", the SDK reports #ERR_INVALID_ARGUMENT.
    ...
    /** @deprecated This method is deprecated, use the \ref wuji::rtc::IRtcEngineEventHandler::onRtmpStreamingStateChanged "onRtmpStreamingStateChanged" callback instead.
     ...
     Reports the result of calling the \ref wuji::rtc::IRtcEngine::removePublishStreamUrl "removePublishStreamUrl" method. (CDN live only.)
 ...
 * 
 * When the `LiveTranscoding` class in the \ref wuji::rtc::IRtcEngine::setLiveTranscoding "setLiveTranscoding" method updates, the SDK triggers the `onTranscodingUpdated` callback to report the update information to the local host.
 * 
 ...
 */
    ...
    */
    ...
    /** Occurs when the local user successfully registers a user account by calling the \ref wuji::rtc::IRtcEngine::registerLocalUserAccount "registerLocalUserAccount" method or joins a channel by calling the \ref wuji::rtc::IRtcEngine::joinChannelWithUserAccount "joinChannelWithUserAccount" method.This callback reports the user ID and user account of the local user.
...
struct RtcEngineContext
{
    ...
    /**
     * The App ID issued to you by Wuji. See [How to get the App ID](https://docs.wuji.io/en/Wuji%20Platform/token#get-an-app-id).
     * Only users in apps with the same App ID can join the same channel and communicate with each other. Use an App ID to create only one `IRtcEngine` instance. To change your App ID, call `release` to destroy the current `IRtcEngine` instance and then call `createWujiRtcEngine`
...
public:
    ...
    /** Occurs when the SDK requests the maximum size of the Metadata.
     ...
     The SDK triggers this callback after you successfully call the \ref wuji::rtc::IRtcEngine::registerMediaMetadataObserver "registerMediaMetadataObserver" method. You need to specify the maximum size of the metadata in the return value of this callback.
    ...
    /** Occurs when the SDK is ready to receive and send metadata.
     ...
     @note Ensure that the size of the metadata does not exceed the value set in the \ref wuji::rtc::IMetadataObserver::getMaxMetadataSize "getMaxMetadataSize" callback.
...
/** IRtcEngine is the base interface class of the Wuji SDK that provides the main Wuji SDK methods invoked by your application.

Enable the Wuji SDK's communication functionality through the creation of an IRtcEngine object, then call the methods of this object.
 */
class IRtcEngine
{
protected:
...
public:
     ...
     * 
     * Unless otherwise specified, all the methods provided by the IRtcEngine class are executed asynchronously. Wuji recommends calling these methods in the same thread.
     * 
    ...
    /** Sets the channel profile of the Wuji IRtcEngine.
     ...
     The Wuji IRtcEngine differentiates channel profiles and applies optimization algorithms accordingly. 
     ...
     @param profile The channel profile of the Wuji IRtcEngine. See #CHANNEL_PROFILE_TYPE
     @return
     - 0: Success.
     - < 0: Failure.
     */
    ...
    /** Sets the role of the user, such as a host or an audience (default), before joining a channel in a live broadcast.
     ...
     In the Live Broadcast profile, when a user switches user roles after joining a channel, a successful \ref wuji::rtc::IRtcEngine::setClientRole "setClientRole" method call triggers the following callbacks:
     - The local client: \ref wuji::rtc::IRtcEngineEventHandler::onClientRoleChanged "onClientRoleChanged"
     - The remote client: \ref wuji::rtc::IRtcEngineEventHandler::onUserJoined "onUserJoined" or \ref wuji::rtc::IRtcEngineEventHandler::onUserOffline "onUserOffline" (BECOME_AUDIENCE)
    ...
    /** Joins a channel with the user ID.
     ...
     A successful \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method call triggers the following callbacks:
     - The local client: \ref wuji::rtc::IRtcEngineEventHandler::onJoinChannelSuccess "onJoinChannelSuccess"
     - The remote client: \ref wuji::rtc::IRtcEngineEventHandler::onUserJoined "onUserJoined" , if the user joining the channel is in the Communication profile, or is a BROADCASTER in the Live Broadcast profile.

     When the connection between the client and Wuji's server is interrupted due to poor network conditions, the SDK tries reconnecting to the server. When the local client successfully rejoins the channel, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onRejoinChannelSuccess "onRejoinChannelSuccess" callback on the local client.
     ...
     - If the user uses a Channel Key, Wuji issues an additional App Certificate for you to generate a user key based on the algorithm and App Certificate for user authentication on the server.
     @param channelId Pointer to the unique channel name for the Wuji RTC session in the string format smaller than 64 bytes. Supported characters:
     - All lowercase English letters: a to z.
     - All uppercase English letters: A to Z.
     - All numeric characters: 0 to 9.
     - The space character.
     ...
     * [Get a temporary token](https://docs.wuji.io/en/Wuji%20Platform/token?platfor%20*%20m=All%20Platforms#get-a-temporary-token).
     * - For high-security requirements: Use the token generated at your
     * server. For details, see
     * [Get a token](https://docs.wuji.io/en/Wuji%20Platform/token?platfor%20*%20m=All%20Platforms#get-a-token).
    ...
    /** Allows a user to leave a channel, such as hanging up or exiting a call.
     ...
     A successful \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method call triggers the following callbacks:
     - The local client: \ref wuji::rtc::IRtcEngineEventHandler::onLeaveChannel "onLeaveChannel"
     - The remote client: \ref wuji::rtc::IRtcEngineEventHandler::onUserOffline "onUserOffline" , if the user leaving the channel is in the Communication channel, or is a BROADCASTER in the Live Broadcast profile.
    ...
    virtual int queryInterface(INTERFACE_ID_TYPE iid, void** inter) = 0;
     ...
     After the user successfully registers a user account, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onLocalUserRegistered "onLocalUserRegistered" callback on the local client,
     reporting the user ID and user account of the local user.

     To join a channel with a user account, you can choose either of the following:

     - Call the \ref wuji::rtc::IRtcEngine::registerLocalUserAccount "registerLocalUserAccount" method to create a user account, and then the \ref wuji::rtc::IRtcEngine::joinChannelWithUserAccount "joinChannelWithUserAccount" method to join the channel.
     - Call the \ref wuji::rtc::IRtcEngine::joinChannelWithUserAccount "joinChannelWithUserAccount" method to join the channel.

     The difference between the two is that for the former, the time elapsed between calling the \ref wuji::rtc::IRtcEngine::joinChannelWithUserAccount "joinChannelWithUserAccount" method
    ...
    */
    ...
    /** Joins the channel with a user account.
     ...
     - The local client: \ref wuji::rtc::IRtcEngineEventHandler::onLocalUserRegistered "onLocalUserRegistered" and \ref wuji::rtc::IRtcEngineEventHandler::onJoinChannelSuccess "onJoinChannelSuccess" .
     The remote client: \ref wuji::rtc::IRtcEngineEventHandler::onUserJoined "onUserJoined" and \ref wuji::rtc::IRtcEngineEventHandler::onUserInfoUpdated "onUserInfoUpdated" , if the user joining the channel is in the Communication profile, or is a BROADCASTER in the Live Broadcast profile.
     ...
     If a user joins the channel with the Wuji Web SDK, ensure that the uid of the user is set to the same parameter type.

     @param token The token generated at your server:
     - For low-security requirements: You can use the temporary token generated at Console. For details, see [Get a temporary toke](https://docs.wuji.io/en/Voice/token?platform=All%20Platforms#get-a-temporary-token).
    ...
    /** Gets the user information by passing in the user account.
     ...
     in a mapping table object (`userInfo`), and triggers the \ref wuji::rtc::IRtcEngineEventHandler::onUserInfoUpdated "onUserInfoUpdated" callback on the local client.

     After receiving the o\ref wuji::rtc::IRtcEngineEventHandler::onUserInfoUpdated "onUserInfoUpdated" callback, you can call this method to get the user ID of the
    ...
    /** Gets the user information by passing in the user ID.
     ...
     caches them in a mapping table object (`userInfo`), and triggers the \ref wuji::rtc::IRtcEngineEventHandler::onUserInfoUpdated "onUserInfoUpdated" callback on the local client.

     After receiving the \ref wuji::rtc::IRtcEngineEventHandler::onUserInfoUpdated "onUserInfoUpdated" callback, you can call this method to get the user account of the remote user
  ...
  virtual int startEchoTest(int intervalInSeconds) = 0;
    ...
    /** Enables the video module.
     ...
     A successful \ref wuji::rtc::IRtcEngine::enableVideo "enableVideo" method call triggers the \ref wuji::rtc::IRtcEngineEventHandler::onUserEnableVideo "onUserEnableVideo" (true) callback on the remote client.
     @note
     - This method affects the internal engine and can be called after the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method.
    ...
    A successful \ref wuji::rtc::IRtcEngine::disableVideo "disableVideo" method call triggers the \ref wuji::rtc::IRtcEngineEventHandler::onUserEnableVideo "onUserEnableVideo" (false) callback on the remote client.
     @note
     - This method affects the internal engine and can be called after the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method.
     - This method resets the internal engine and takes some time to take effect. We recommend using the following API methods to control the video engine modules separately:
         - \ref IRtcEngine::enableLocalVideo "enableLocalVideo": Whether to enable the camera to create the local video stream.
    ...
    /** Sets the camera capture configuration.
     ...
     @note Call this method before enabling the local camera. That said, you can call this method before calling \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel", \ref wuji::rtc::IRtcEngine::enableVideo "enableVideo", or \ref IRtcEngine::enableLocalVideo "enableLocalVideo", depending on which method you use to turn on your local camera.
    ...
    The audio mode is enabled by default.
     ...
     @note
     - This method affects the internal engine and can be called after the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method. You can call this method either before or after joining a channel.
    ...
    */
    ...
    The SDK triggers the \ref IRtcEngineEventHandler::onMicrophoneEnabled "onMicrophoneEnabled" callback once the local audio function is disabled or enabled.
     ...
     @note
     This method is different from the \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream" method:
        - \ref wuji::rtc::IRtcEngine::enableLocalAudio "enableLocalAudio": Disables/Re-enables the local audio capturing and processing. 
        ...
        - \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream": Sends/Stops sending the local audio streams.
    ...
    /** Disables the audio module.
     ...
     @note
     - This method affects the internal engine and can be called after the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method. You can call this method either before or after joining a channel.
     - This method resets the internal engine and takes some time to take effect. We recommend using the \ref wuji::rtc::IRtcEngine::enableLocalAudio "enableLocalAudio" and \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream" methods to capture, process, and send the local audio streams.
	...
	/** Sets the audio parameters and application scenarios.
     ...
     [What is the difference between the in-call volume and the media volume?](https://docs.wuji.io/en/faq/system_volume).
    ...
    /** Stops/Resumes sending the local audio stream.
     ...
     A successful \ref wuji::rtc::IRtcEngine::muteLocalAudioStream "muteLocalAudioStream" method call triggers the \ref wuji::rtc::IRtcEngineEventHandler::onUserMuteAudio "onUserMuteAudio" callback on the remote client.
     @note 
     - When @p mute is set as @p true, this method does not disable the microphone, which does not affect any ongoing recording.
     - If you call \ref wuji::rtc::IRtcEngine::setChannelProfile "setChannelProfile" after this method, the SDK resets whether or not to mute the local audio according to the channel profile and user role. Therefore, we recommend calling this method after the `setChannelProfile` method.
    ...
    /** Stops/Resumes receiving all remote users' audio streams by default.
     ...
     @note If you want to resume receiving the audio stream, call \ref wuji::rtc::IRtcEngine::muteRemoteAudioStream "muteRemoteAudioStream (false)", 
	...
	/** Stops/Resumes receiving a specified remote user's audio stream.
	 ...
	 @note If you called the \ref wuji::rtc::IRtcEngine::muteAllRemoteAudioStreams "muteAllRemoteAudioStreams" method and set @p mute as @p true to stop receiving all remote users' audio streams, call the *muteAllRemoteAudioStreams* method and set @p mute as @p false before calling this method. The *muteAllRemoteAudioStreams* method sets all remote audio streams, while the *muteRemoteAudioStream* method sets a specified remote audio stream.
    ...
    /** Stops/Resumes sending the local video stream.
     ...
     A successful \ref wuji::rtc::IRtcEngine::muteLocalVideoStream "muteLocalVideoStream" method call triggers the \ref wuji::rtc::IRtcEngineEventHandler::onUserMuteVideo "onUserMuteVideo" callback on the remote client.

     @note
     - When set to *true*, this method does not disable the camera which does not affect the retrieval of the local video streams. This method executes faster than the \ref wuji::rtc::IRtcEngine::enableLocalVideo "enableLocalVideo" method which controls the sending of the local video stream.
     - If you call \ref wuji::rtc::IRtcEngine::setChannelProfile "setChannelProfile" after this method, the SDK resets whether or not to mute the local video according to the channel profile and user role. Therefore, we recommend calling this method after the `setChannelProfile` method.
    ...
    /** Enables/Disables the local video capture.
     ...
     After you call the \ref wuji::rtc::IRtcEngine::enableVideo "enableVideo" method, the local video capturer is enabled by default. You can call \ref wuji::rtc::IRtcEngine::enableLocalVideo "enableLocalVideo(false)" to disable the local video capturer. If you want to re-enable it, call \ref wuji::rtc::IRtcEngine::enableLocalVideo "enableLocalVideo(true)".

     After the local video capturer is successfully disabled or re-enabled, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onUserEnableLocalVideo "onUserEnableLocalVideo" callback on the remote client.

     @note This method affects the internal engine and can be called after the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method.
    ...
    /** Stops/Resumes receiving the video stream from a specified remote user.
     ...
     @note If you called the \ref wuji::rtc::IRtcEngine::muteAllRemoteVideoStreams "muteAllRemoteVideoStreams" method and set @p mute as @p true to stop receiving all remote video streams, call the *muteAllRemoteVideoStreams* method and set @p mute as @p false before calling this method.
    ...
    /** Sets the stream type of the remote video.
     ...
     The method result returns in the \ref wuji::rtc::IRtcEngineEventHandler::onApiCallExecuted "onApiCallExecuted" callback.
    ...
    /** Enables the \ref wuji::rtc::IRtcEngineEventHandler::onAudioVolumeIndication "onAudioVolumeIndication" callback at a set time interval to report on which users are speaking and the speakers' volume.

     Once this method is enabled, the SDK returns the volume indication in the \ref wuji::rtc::IRtcEngineEventHandler::onAudioVolumeIndication "onAudioVolumeIndication" callback at the set time interval, whether or not any user is speaking in the channel.
    ...
    /** @deprecated Starts an audio recording.
     ...
     This method is usually called after the \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel" method.
     The recording automatically stops when the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method is called.
    ...
    /** Stops an audio recording on the client.
     ...
     You can call this method before calling the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method else, the recording automatically stops when the \ref wuji::rtc::IRtcEngine::leaveChannel "leaveChannel" method is called.
    ...
    /** Starts playing and mixing the music file.
     ...
     When the audio mixing file playback finishes after calling this method, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onAudioMixingFinished "onAudioMixingFinished" callback.

     A successful \ref wuji::rtc::IRtcEngine::startAudioMixing "startAudioMixing" method call triggers the \ref wuji::rtc::IRtcEngineEventHandler::onAudioMixingStateChanged "onAudioMixingStateChanged" (PLAY) callback on the local client.

     When the audio mixing file playback finishes, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onAudioMixingStateChanged "onAudioMixingStateChanged" (STOPPED) callback on the local client.
     @note
    ...
    /** Adjusts the volume during audio mixing.
     ...
     @note Calling this method does not affect the volume of audio effect file playback invoked by the \ref wuji::rtc::IRtcEngine::playEffect "playEffect" method.
    ...
    /** Enables/Disables stereo panning for remote users.
     ...
     Ensure that you call this method before joinChannel to enable stereo panning for remote users so that the local user can track the position of a remote user by calling \ref wuji::rtc::IRtcEngine::setRemoteVoicePosition "setRemoteVoicePosition".
    ...
    /** Sets the sound position and gain of a remote user.
     ...
     @note
     - For this method to work, enable stereo panning for remote users by calling the \ref wuji::rtc::IRtcEngine::enableSoundPositionIndication "enableSoundPositionIndication" method before joining a channel.
    ...
    /**  Sets the local voice reverberation.
     ...
     v2.4.0 adds the \ref wuji::rtc::IRtcEngine::setLocalVoiceReverbPreset "setLocalVoiceReverbPreset" method, a more user-friendly method for setting the local voice reverberation. You can use this method to set the local reverberation effect, such as pop music, R&B, rock music, and hip-hop.
    ...
    /** Sets the local voice changer option.
     ...
     @note
     - To achieve better voice effect quality, Wuji recommends setting the profile parameter in `setAudioProfile` as `AUDIO_PROFILE_MUSIC_HIGH_QUALITY(4)` or `AUDIO_PROFILE_MUSIC_HIGH_QUALITY_STEREO(5)`.
     ...
     * - When calling this method with `AUDIO_VIRTUAL_STEREO`, Wuji recommends setting the `profile` parameter in `setAudioProfile` as `AUDIO_PROFILE_MUSIC_HIGH_QUALITY_STEREO(5)`.
     * - This method works best with the human voice, and Wuji does not recommend using it for audio containing music and a human voice.
     ...
     * For detailed considerations, see the advanced guide *Voice Changer and Reverberation*.
     ...
     To achieve better voice effects, Wuji recommends the enumeration whose name begins with `AUDIO_REVERB_FX`.
    ...
    /** Specifies an SDK output log file.
     ...
     @note
     - The default log file is located at: `C: \Users\<user_name>\AppData\Local\Wuji\<process_name>`.
     - Ensure that you call this method immediately after calling the \ref wuji::rtc::IRtcEngine::initialize "initialize" method, otherwise the output log may not be complete.
    ...
    /** 
     ...
     You must call this method before calling the \ref wuji::rtc::IRtcEngine::startPreview "startPreview" method, otherwise the mirror mode will not work.

     @warning
     - Call this method after calling the \ref wuji::rtc::IRtcEngine::setupLocalVideo "setupLocalVideo" method to initialize the local video view.
    ...
    /** Sets the external audio source. Please call this method before \ref wuji::rtc::IRtcEngine::joinChannel "joinChannel".
     ...
     * the \ref wuji::media::IMediaEngine::pullAudioFrame "pullAudioFrame" method to pull the remote audio data, process
     * it, and play it with the audio effects that you want.
     *
     * @note
     * Once you enable the external audio sink, the app will not retrieve any
     * audio data from the
     * \ref wuji::media::IAudioFrameObserver::onPlaybackAudioFrame "onPlaybackAudioFrame" callback.
     *
     * @param enabled
    ...
    /** Sets the audio recording format for the \ref wuji::media::IAudioFrameObserver::onRecordAudioFrame "onRecordAudioFrame" callback. 
    ...
    /** Sets the audio playback format for the \ref wuji::media::IAudioFrameObserver::onPlaybackAudioFrame "onPlaybackAudioFrame" callback.
    ...
    /** Sets the mixed audio format for the \ref wuji::media::IAudioFrameObserver::onMixedAudioFrame "onMixedAudioFrame" callback.
    ...
    /** 
     ...
     @note 
     - This method applies only to the Live-broadcast profile. In the Communication profile, interoperability with the Wuji Web SDK is enabled by default.
    ...
    /** Sets the fallback option for the published video stream based on the network conditions.
     ...
     When the published video stream falls back to audio only or when the audio-only stream switches back to the video, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onLocalPublishFallbackToAudioOnly "onLocalPublishFallbackToAudioOnly" callback.

     @note Wuji does not recommend using this method for CDN live streaming, because the remote CDN live user will have a noticeable lag when the published video stream falls back to audio only.
    ...
    /** Sets the fallback option for the remotely subscribed video stream based on the network conditions.
     ...
     When the remotely subscribed video stream falls back to audio only or when the audio-only stream switches back to the video stream, the SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onRemoteSubscribeFallbackToAudioOnly "onRemoteSubscribeFallbackToAudioOnly" callback.
    ...
    /** Shares the whole or part of a window by specifying the window ID.
     ...
     Since v3.0.0, this method supports sharing with common Windows platforms. Wuji tests the mainstream Windows applications, see details as follows:
    ...
    */
    ...
    /** **DEPRECATED** Starts screen sharing.
     ...
     - \ref wuji::rtc::IRtcEngine::startScreenCaptureByDisplayId "startScreenCaptureByDisplayId"
     - \ref wuji::rtc::IRtcEngine::startScreenCaptureByScreenRect "startScreenCaptureByScreenRect"
     - \ref wuji::rtc::IRtcEngine::startScreenCaptureByWindowId "startScreenCaptureByWindowId"
    ...
    */
    ...
    /**  Enables the network connection quality test.
     ...
     - If you call this method to test the last-mile quality, the SDK consumes the bandwidth of a video stream, whose bitrate corresponds to the bitrate you set in the \ref wuji::rtc::IRtcEngine::setVideoEncoderConfiguration "setVideoEncoderConfiguration" method. After you join the channel, whether you have called the `disableLastmileTest` method or not, the SDK automatically stops consuming the bandwidth.
    ...
    /** Retrieves the warning or error description.
     ...
     @param code Warning code or error code returned in the \ref wuji::rtc::IRtcEngineEventHandler::onWarning "onWarning" or \ref wuji::rtc::IRtcEngineEventHandler::onError "onError" callback.
    ...
    /** Sets the built-in encryption mode.
     ...
     The Wuji SDK supports built-in encryption, which is set to the @p aes-128-xts mode by default. Call this method to use other encryption modes.
    ...
    /** Registers a packet observer.
     ...
     The Wuji SDK allows your application to register a packet observer to receive callbacks for voice or video packet transmission.
     ...
     - When you use CDN live streaming, recording or storage functions, Wuji doesn't recommend calling this method.
    ...
    /** Sends data stream messages to all users in a channel.
     ...
     A successful \ref wuji::rtc::IRtcEngine::sendStreamMessage "sendStreamMessage" method call triggers the 
     \ref wuji::rtc::IRtcEngineEventHandler::onStreamMessage "onStreamMessage" callback on the remote client, from which the remote user gets the stream message.

     A failed \ref wuji::rtc::IRtcEngine::sendStreamMessage "sendStreamMessage" method call triggers the
      \ref wuji::rtc::IRtcEngineEventHandler::onStreamMessage "onStreamMessage" callback on the remote client.
    ...
    /** Publishes the local stream to a specified CDN live RTMP address.  (CDN live only.)
     ...
     The \ref wuji::rtc::IRtcEngine::addPublishStreamUrl "addPublishStreamUrl" method call triggers the \ref wuji::rtc::IRtcEngineEventHandler::onRtmpStreamingStateChanged "onRtmpStreamingStateChanged" callback on the local client to report the state of adding a local stream to the CDN.
     @note
    ...
    /** Removes an RTMP stream from the CDN. (CDN live only.)
     ...
     The \ref wuji::rtc::IRtcEngine::removePublishStreamUrl "removePublishStreamUrl" method call triggers the \ref wuji::rtc::IRtcEngineEventHandler::onRtmpStreamingStateChanged "onRtmpStreamingStateChanged" callback on the local client to report the state of removing an RTMP stream from the CDN.
     @note
    ...
    /** Sets the video layout and audio settings for CDN live. (CDN live only.)
     ...
     The SDK triggers the \ref wuji::rtc::IRtcEngineEventHandler::onTranscodingUpdated "onTranscodingUpdated" callback when you call the `setLiveTranscoding` method to update the transcoding setting.
    ...
    /** **DEPRECATED** Adds a watermark image to the local video or CDN live stream.
     ...
     This method is deprecated from v2.9.1. Use \ref wuji::rtc::IRtcEngine::addVideoWatermark(const char* watermarkUrl, const WatermarkOptions& options) "addVideoWatermark"2 instead.
     ...
     - The Wuji SDK supports adding only one watermark image onto a local video or CDN live stream. The newly added watermark image replaces the previous one.
    ...
    /** Adds a watermark image to the local video.
     ...
     and the recording device can see and capture it. Wuji supports adding only one watermark image onto the local video, and the newly watermark image replaces the previous one.
     ...
     @note
     - Ensure that you have called the \ref wuji::rtc::IRtcEngine::enableVideo "enableVideo" method to enable the video module before calling this method.
     - If you only want to add a watermark image to the local video for the audience in the CDN live broadcast channel to see and capture, you can call this method or the \ref wuji::rtc::IRtcEngine::setLiveTranscoding "setLiveTranscoding" method.
     - This method supports adding a watermark image in the PNG file format only. Supported pixel formats of the PNG image are RGBA, RGB, Palette, Gray, and Alpha_gray.
     - If the dimensions of the PNG image differ from your settings in this method, the image will be cropped or zoomed to conform to your settings.
     - If you have enabled the local video preview by calling the \ref wuji::rtc::IRtcEngine::startPreview "startPreview" method, you can use the `visibleInPreview` member in the WatermarkOptions class to set whether or not the watermark is visible in preview.
    ...
    /** Removes the watermark image from the video stream added by the \ref wuji::rtc::IRtcEngine::addVideoWatermark(const char* watermarkUrl, const WatermarkOptions& options) "addVideoWatermark" method.
    ...
    */
    ...
    The \ref IRtcEngineEventHandler::onStreamPublished "onStreamPublished" callback returns the inject status. If this method call is successful, the server pulls the voice or video stream and injects it into a live channel. This is applicable to scenarios where all audience members in the channel can watch a live show and interact with each other.
     ...
     The \ref wuji::rtc::IRtcEngine::addInjectStreamUrl "addInjectStreamUrl" method call triggers the following callbacks:
    - The local client:
      - \ref wuji::rtc::IRtcEngineEventHandler::onStreamInjectedStatus "onStreamInjectedStatus" , with the state of the injecting the online stream.
      - \ref wuji::rtc::IRtcEngineEventHandler::onUserJoined "onUserJoined" (uid: 666), if the method call is successful and the online media stream is injected into the channel.
    - The remote client:
      - \ref wuji::rtc::IRtcEngineEventHandler::onUserJoined "onUserJoined" (uid: 666), if the method call is successful and the online media stream is injected into the channel.
     ...
     - < 0: Failure.
        ...
        - #ERR_NOT_SUPPORTED (4): The channel profile is not live broadcast. Call the \ref wuji::rtc::IRtcEngine::setChannelProfile "setChannelProfile" method and set the channel profile to live broadcast before calling this method.
    ...
    /** Registers the metadata observer.
     ...
     Registers the metadata observer. You need to implement the IMetadataObserver class and specify the metadata type in this method. A successful call of this method triggers the \ref wuji::rtc::IMetadataObserver::getMaxMetadataSize "getMaxMetadataSize" callback.
...
public:
    ...
    AParameter(IRtcEngine* engine) { initialize(engine); }
    AParameter(IRtcEngineParameter* p) :wuji::util::AutoPtr<IRtcEngineParameter>(p) {}
private:
    bool initialize(IRtcEngine* engine)
    {
        IRtcEngineParameter* p = NULL;
        if (engine && !engine->queryInterface(WUJI_IID_RTC_ENGINE_PARAMETER, (void**)&p))
            reset(p);
        return p != NULL;
    }
};
...
public:
    ...
    int setPlaybackDeviceVolume(int volume) {// [0,255]
        ...
        if (engine_->queryInterface(WUJI_IID_AUDIO_DEVICE_MANAGER, (void**)&p) < 0) {
    ...
    int stopAudioRecording() {
        return engine_->stopAudioRecording();
    }
    ...
    int stopAudioMixing() {
        return engine_->stopAudioMixing();
    }

    
    int pauseAudioMixing() {
        return engine_->pauseAudioMixing();
    }

    
    int resumeAudioMixing() {
        return engine_->resumeAudioMixing();
    }
    ...
    int getAudioMixingDuration() {
        return engine_->getAudioMixingDuration();
    }
    ...
    int getEffectsVolume() {
        return engine_->getEffectsVolume();
    }

    
    int setEffectsVolume(int volume) {
        return engine_->setEffectsVolume(volume);
    }
    ...
    int playEffect(int soundId, const char* filePath, int loopCount, double pitch, double pan, int gain, bool publish = false) {
        return engine_->playEffect(soundId, filePath, loopCount, pitch, pan, gain, publish);
    }

    
    int stopEffect(int soundId) {
        return engine_->stopEffect(soundId);
    }

   
    int stopAllEffects() {
        return engine_->stopAllEffects();
    }
    ...
    int unloadEffect(int soundId) {
        return engine_->unloadEffect(soundId);
    }

    
    int pauseEffect(int soundId) {
        return engine_->pauseEffect(soundId);
    }

    
    int pauseAllEffects() {
        return engine_->pauseAllEffects();
    }

    
    int resumeEffect(int soundId) {
        return engine_->resumeEffect(soundId);
    }

    
    int resumeAllEffects() {
        return engine_->resumeAllEffects();
    }
     ...
     */
    int pauseAudio() {
        return engine_->disableAudio();
    }

    
    int resumeAudio() {
        return engine_->enableAudio();
    }

    
    int setHighQualityAudioParameters(bool fullband, bool stereo, bool fullBitrate) {
        return engine_->setHighQualityAudioParameters(fullband, stereo, fullBitrate);
    }
    ...
    int setLogFile(const char* filePath) {
        return engine_->setLogFile(filePath);
    }
    ...
    int setRecordingAudioFrameParameters(int sampleRate, int channel, RAW_AUDIO_FRAME_OP_MODE_TYPE mode, int samplesPerCall) {
        return engine_->setRecordingAudioFrameParameters(sampleRate, channel, mode, samplesPerCall);
    }
    
    int setPlaybackAudioFrameParameters(int sampleRate, int channel, RAW_AUDIO_FRAME_OP_MODE_TYPE mode, int samplesPerCall) {
        return engine_->setPlaybackAudioFrameParameters(sampleRate, channel, mode, samplesPerCall);
    }
 ...
 * 
 * @note The Wuji RTC Native SDK supports creating only one `IRtcEngine` object for an app for now.
 * 
