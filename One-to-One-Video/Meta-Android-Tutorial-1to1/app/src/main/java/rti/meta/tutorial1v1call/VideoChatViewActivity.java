package rti.meta.tutorial1v1call;
...
public class VideoChatViewActivity extends AppCompatActivity {
    ...
    private void initializeEngine() {
        try {
            mRtcEngine = RtcEngine.create(getBaseContext(), getString(R.string.meta_app_id), mRtcEventHandler);
        } catch (Exception e) {
    ...
    private void joinChannel() {
        ...
        String token = getString(R.string.meta_access_token);
        if (TextUtils.isEmpty(token) || TextUtils.equals(token, "#YOUR ACCESS TOKEN#")) {
            token = null; // default, no token
        }
        mRtcEngine.joinChannel(token, "default_channel", "Extra Optional Data", 0);
    }
