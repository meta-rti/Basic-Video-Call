...
#ifndef IMetaRtcChannel_h
#define IMetaRtcChannel_h
...
struct ChannelMediaOptions {
    /** Determines whether to subscribe to audio streams when the user joins the channel:
     ...
     This member serves a similar function to the \ref meta::rtc::IChannel::muteAllRemoteAudioStreams "muteAllRemoteAudioStreams" method. After joining the channel, 
    ...
    /** Determines whether to subscribe to video streams when the user joins the channel:
     ...
     This member serves a similar function to the \ref meta::rtc::IChannel::muteAllRemoteVideoStreams "muteAllRemoteVideoStreams" method. After joining the channel, 
...
public:
    ...
    /** Occurs when a user leaves the channel.
     ...
     This callback notifies the application that a user leaves the channel when the application calls the \ref meta::rtc::IChannel::leaveChannel "leaveChannel" method.
    ...
    /** Occurs when a remote user (Communication)/ host (Live Broadcast) joins the channel.
     ...
     - A remote user/host joins the channel by calling the \ref meta::rtc::IChannel::joinChannel "joinChannel" method.
     - A remote user switches the user role to the host by calling the \ref meta::rtc::IChannel::setClientRole "setClientRole" method after joining the channel.
     - A remote user/host rejoins the channel after a network interruption.
     - The host injects an online media stream into the channel by calling the \ref meta::rtc::IChannel::addInjectStreamUrl "addInjectStreamUrl" method.
    ...
    /** Occurs when a remote user (Communication)/host (Live Broadcast) leaves the channel.
     ...
     - Drop offline: When no data packet of the user or host is received for a certain period of time, the SDK assumes that the user/host drops offline. Unreliable network connections may lead to false detections, so we recommend using the Meta RTM SDK for more reliable offline detection.
    ...
    /** Occurs when the SDK cannot reconnect to Meta's edge server 10 seconds after its connection to the server is interrupted.
     ...
     This callback is different from \ref meta::rtc::IRtcEngineEventHandler::onConnectionInterrupted "onConnectionInterrupted":
    ...
    /** Occurs when the token expires.
     ...
     After a token is specified by calling the \ref IChannel::joinChannel "joinChannel" method, if the SDK losses connection with the Meta server due to network issues, the token may expire after a certain period of time and a new token may be required to reconnect to the server.
    ...
    /** Reports the last mile network quality of each user in the channel once every two seconds.
     ...
     Last mile refers to the connection between the local device and Meta's edge server. This callback reports once every two seconds the last mile network conditions of each user in the channel. If a channel includes multiple users, the SDK triggers this callback as many times.
    ...
    /** Reports the statistics of the audio stream from each remote user/host.
     ...
     This callback replaces the \ref meta::rtc::IRtcEngineEventHandler::onAudioQuality "onAudioQuality" callback.
    ...
    */
    ...
    /** Occurs when the remote video state changes.
     ...
     @param elapsed Time elapsed (ms) from the local user calling the
     \ref meta::rtc::IChannel::joinChannel "joinChannel" method until the
    ...
    /** Occurs when the local user receives the data stream from the remote user within five seconds.
     ...
     The SDK triggers this callback when the local user receives the stream message that the remote user sends by calling the \ref meta::rtc::IChannel::sendStreamMessage "sendStreamMessage" method.
    ...
    */
    ...
    /** Occurs when the local user does not receive the data stream from the remote user within five seconds.
     ...
     The SDK triggers this callback when the local user fails to receive the stream message that the remote user sends by calling the \ref meta::rtc::IChannel::sendStreamMessage "sendStreamMessage" method.
    ...
    /**
     ...
     The SDK triggers this callback to report the result of the local user calling the \ref meta::rtc::IChannel::addPublishStreamUrl "addPublishStreamUrl" or \ref meta::rtc::IChannel::removePublishStreamUrl "removePublishStreamUrl" method.
    ...
    virtual void onRtmpStreamingStateChanged(IChannel *rtcChannel, const char *url, RTMP_STREAM_PUBLISH_STATE state, RTMP_STREAM_PUBLISH_ERROR errCode) {
     ...
     When the `LiveTranscoding` class in the \ref meta::rtc::IChannel::setLiveTranscoding "setLiveTranscoding" method updates, the SDK triggers the `onTranscodingUpdated` callback to report the update information to the local host.
...
public:
    ...
    /** Joins the channel with a user ID.
     ...
     - In situations not requiring high security: You can use the temporary token generated at Console. For details, see [Get a temporary token](https://docs.meta.io/en/Meta%20Platform/token?platfor%20*%20m=All%20Platforms#get-a-temporary-token).
     - In situations requiring high security: Set it as the token generated at your server. For details, see [Generate a token](https://docs.meta.io/en/Meta%20Platform/token?platfor%20*%20m=All%20Platforms#get-a-token).
     ...
     @param uid The user ID. A 32-bit unsigned integer with a value ranging from 1 to (232-1). This parameter must be unique. If `uid` is not assigned (or set as `0`), the SDK assigns a `uid` and reports it in the \ref meta::rtc::IChannelEventHandler::onJoinChannelSuccess "onJoinChannelSuccess" callback. The app must maintain this user ID.
     @param options The channel media options: \ref meta::rtc::ChannelMediaOptions::ChannelMediaOptions "ChannelMediaOptions"
    ...
    /** Joins the channel with a user account.
     ...
     - The local client: \ref meta::rtc::IRtcEngineEventHandler::onLocalUserRegistered "onLocalUserRegistered" and \ref meta::rtc::IChannelEventHandler::onJoinChannelSuccess "onJoinChannelSuccess" .
     - The remote client: \ref meta::rtc::IChannelEventHandler::onUserJoined "onUserJoined" and \ref meta::rtc::IRtcEngineEventHandler::onUserInfoUpdated "onUserInfoUpdated" , if the user joining the channel is in the Communication profile, or is a BROADCASTER in the Live Broadcast profile.
     ...
     If a user joins the channel with the Meta Web SDK, ensure that the uid of the user is set to the same parameter type.

     @param token The token generated at your server:
     - For low-security requirements: You can use the temporary token generated at Console. For details, see [Get a temporary toke](https://docs.meta.io/en/Voice/token?platform=All%20Platforms#get-a-temporary-token).
     - For high-security requirements: Set it as the token generated at your server. For details, see [Get a token](https://docs.meta.io/en/Voice/token?platform=All%20Platforms#get-a-token).
    ...
    /** Allows a user to leave a channel, such as hanging up or exiting a call.
     ...
     A successful \ref meta::rtc::IChannel::leaveChannel "leaveChannel" method call triggers the following callbacks:
     - The local client: \ref meta::rtc::IChannelEventHandler::onLeaveChannel "onLeaveChannel"
     - The remote client: \ref meta::rtc::IChannelEventHandler::onUserOffline "onUserOffline" , if the user leaving the channel is in the Communication channel, or is a BROADCASTER in the Live Broadcast profile.
    ...
    /** Publishes the local stream to the channel.
     ...
     - In a Live Broadcast channel, only a broadcaster can call this method. To switch the client role, call \ref meta::rtc::IChannel::setClientRole "setClientRole" of the current `IChannel` object.
    ...
    /** Sets the built-in encryption mode.
     ...
     The Meta SDK supports built-in encryption, which is set to the `aes-128-xts` mode by default. Call this method to use other encryption modes.
    ...
    /** Registers a packet observer.
     ...
     The Meta SDK allows your application to register a packet observer to receive callbacks for voice or video packet transmission.
     ...
     - When you use CDN live streaming, recording or storage functions, Meta doesn't recommend calling this method.
    ...
    /** Registers the metadata observer.
     ...
     Registers the metadata observer. You need to implement the IMetadataObserver class and specify the metadata type in this method. A successful call of this method triggers the \ref meta::rtc::IMetadataObserver::getMaxMetadataSize "getMaxMetadataSize" callback.
    ...
    /** Sets the role of the user, such as a host or an audience (default), before joining a channel in a live broadcast.
     ...
     In the Live Broadcast profile, when a user switches user roles after joining a channel, a successful \ref meta::rtc::IChannel::setClientRole "setClientRole" method call triggers the following callbacks:
     - The local client: \ref meta::rtc::IChannelEventHandler::onClientRoleChanged "onClientRoleChanged"
     - The remote client: \ref meta::rtc::IChannelEventHandler::onUserJoined "onUserJoined" or \ref meta::rtc::IChannelEventHandler::onUserOffline "onUserOffline" (BECOME_AUDIENCE)
    ...
    /** Prioritizes a remote user's stream.
     ...
     @note The Meta SDK supports setting `serPriority` as high for one user only.
    ...
    /** Sets the sound position and gain of a remote user.
     ...
     @note
     - For this method to work, enable stereo panning for remote users by calling the \ref meta::rtc::IRtcEngine::enableSoundPositionIndication "enableSoundPositionIndication" method before joining a channel.
    ...
    /** Updates the display mode of the video view of a remote user.
     ...
     @note
     - Call this method after calling the \ref meta::rtc::IRtcEngine::setupRemoteVideo "setupRemoteVideo" method to initialize the remote video view.
    ...
    /** Sets whether to receive all remote audio streams by default. 
     ...
     @note If you want to resume receiving the audio stream, call \ref meta::rtc::IChannel::muteRemoteAudioStream "muteRemoteAudioStream (false)", 
    ...
    /** Sets whether to receive all remote video streams by default. 
     ...
     \ref meta::rtc::IChannel::muteRemoteVideoStream "muteRemoteVideoStream (false)", 
    ...
    /** Adjust the playback volume of the specified remote user.
     ...
     After joining a channel, call \ref meta::rtc::IRtcEngine::adjustPlaybackSignalVolume "adjustPlaybackSignalVolume" to adjust the playback volume of different remote users, 
     ...
     @param userId The user ID, which should be the same as the `uid` of \ref meta::rtc::IChannel::joinChannel "joinChannel"
    ...
    /** Stops/Resumes receiving a specified remote user's audio stream.
	 ...
	 @note If you called the \ref meta::rtc::IChannel::muteAllRemoteAudioStreams "muteAllRemoteAudioStreams" method and set `mute` as `true` to stop 
    ...
    /** Stops/Resumes receiving the video stream from a specified remote user.
     ...
     @note If you called the \ref meta::rtc::IChannel::muteAllRemoteVideoStreams "muteAllRemoteVideoStreams" method and 
    ...
    /** Sets the stream type of the remote video.
     ...
     \ref meta::rtc::IRtcEngine::enableDualStreamMode "enableDualStreamMode" (false), 
     ...
     The method result returns in the \ref meta::rtc::IRtcEngineEventHandler::onApiCallExecuted "onApiCallExecuted" callback.
    ...
    /** Sends data stream messages to all users in a channel.
     ...
     A successful \ref meta::rtc::IChannel::sendStreamMessage "sendStreamMessage" method call triggers 
     the \ref meta::rtc::IChannelEventHandler::onStreamMessage "onStreamMessage" callback on the remote client, from which the remote user gets the stream message.

     A failed \ref meta::rtc::IChannel::sendStreamMessage "sendStreamMessage" method call triggers 
     the \ref meta::rtc::IChannelEventHandler::onStreamMessageError "onStreamMessage" callback on the remote client.
     ...
     - Ensure that you have created the data stream using \ref meta::rtc::IChannel::createDataStream "createDataStream" before calling this method.
    ...
    /** Publishes the local stream to a specified CDN live RTMP address.  (CDN live only.)
     ...
     The \ref meta::rtc::IChannel::addPublishStreamUrl "addPublishStreamUrl" method call triggers 
     the \ref meta::rtc::IChannelEventHandler::onRtmpStreamingStateChanged "onRtmpStreamingStateChanged" callback on the local client 
    ...
    /** Removes an RTMP stream from the CDN. 
     ...
     The \ref meta::rtc::IChannel::removePublishStreamUrl "removePublishStreamUrl" method call triggers 
     the \ref meta::rtc::IChannelEventHandler::onRtmpStreamingStateChanged "onRtmpStreamingStateChanged" callback on the local client to report the state of removing an RTMP stream from the CDN.
    ...
    /** Sets the video layout and audio settings for CDN live. (CDN live only.)
     ...
     The SDK triggers the \ref meta::rtc::IChannelEventHandler::onTranscodingUpdated "onTranscodingUpdated" callback when you 
    ...
    This is applicable to scenarios where all audience members in the channel can watch a live show and interact with each other.
     ...
     The \ref meta::rtc::IChannel::addInjectStreamUrl "addInjectStreamUrl" method call triggers the following callbacks:
    - The local client:
      - \ref meta::rtc::IChannelEventHandler::onStreamInjectedStatus "onStreamInjectedStatus" , with the state of the injecting the online stream.
      - \ref meta::rtc::IChannelEventHandler::onUserJoined "onUserJoined" (uid: 666), if the method call is successful and the online media stream is injected into the channel.
    - The remote client:
      - \ref meta::rtc::IChannelEventHandler::onUserJoined "onUserJoined" (uid: 666), if the method call is successful and the online media stream is injected into the channel.
     ...
     * \ref meta::rtc::IChannelEventHandler::onChannelMediaRelayStateChanged
     *  "onChannelMediaRelayStateChanged" and
     * \ref meta::rtc::IChannelEventHandler::onChannelMediaRelayEvent
    ...
    virtual int pushVideoFrame(meta::media::ExternalVideoFrame *frame) = 0;
};
/** @since v3.0.0 
...
public:
    ...
    /** Creates and gets an `IChannel` object.
     ...
     call the \ref meta::rtc::IChannel::joinChannel "joinChannel" method of each created `IChannel` object.
     
     After joining multiple channels, you can simultaneously subscribe to streams of all the channels, but publish a stream in only one channel at one time.
     @param channelId The unique channel name for an Meta RTC session. It must be in the string format and not exceed 64 bytes in length. Supported character scopes are:
     - All lowercase English letters: a to z. 
     - All uppercase English letters: A to Z. 
     - All numeric characters: 0 to 9. 
     - The space character. 
